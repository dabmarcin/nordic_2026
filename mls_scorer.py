...
LEAGUE_DIRS = {
    'mls': (MLS_DIR, MODELS_MLS, MLS_SCORER_DIR),
}
...
# Empirical win rates for contrarian models (MLS)
P_CORNERS_OVER_MLS = 0.65
P_OVER25_FORCED_MLS = 0.68
...
def predict_model(model_name, model, imputer, features_dict, row, rejected, match_label):
    ...
    if model_name == 'corners':
        # Always play Over 9.5 corners for MLS
    ...
    if model_name == 'over25' and score < 50.0:
        ...
        p = P_OVER25_FORCED_MLS  # Use empirical probability
    else:
...
def main():
    ...
    mls = daily_df[daily_df['competition_id'].isin(comp_filter)].copy()

    id_to_league = {v: k for k, v in ACTIVE_LEAGUES.items()}

    print('══════════════════════════════════════════════════════')
    print(f'MLS SCORER — {date_str} ({day_name_pl})')
    print('══════════════════════════════════════════════════════')

    n_per_league = {lg: int((mls['competition_id'] == cid).sum())
                    for lg, cid in ACTIVE_LEAGUES.items()}
    print(f"Meczów: MLS={n_per_league['mls']}")

    if len(mls) == 0:
        print('Brak meczów MLS na wybrany dzień.')
        sys.exit(0)
    ...
    if args.debug:
        ...
        debug_path = os.path.join(REPORTS_DIR, f'debug_mls_{date_str}_{ts}.json')
